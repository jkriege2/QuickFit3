...
if [ "$CLASSNAME" == "" ]; then
	echo -n "class name for QFImporter plugin implementation: "
	read -e CLASSNAME
else
	echo -n "class name for QFImporter plugin implementation: " $CLASSNAME "
"
fi
...
if [ "$IMPORTERCLASSNAME" == "" ]; then
	echo -n "class name for first importer: "
	read -e IMPORTERCLASSNAME
else
	echo -n "class name for first importer: " $IMPORTERCLASSNAME "
"
fi
...
if [ "$IMPORTERNAME" == "" ]; then
	...
	echo -n "ID for first importer: " $IMPORTERNAME "
"
fi
...
replace_in_all 's/doxygen_GROUPNAME/qf3importerplugins_'$TARGETNAME'/g'
