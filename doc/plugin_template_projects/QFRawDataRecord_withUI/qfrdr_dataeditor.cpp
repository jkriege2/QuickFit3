...
void QFRDRDataEditor::connectWidgets(QFRawDataRecord* current, QFRawDataRecord* old) {
    ...
    if (m) {
		...
		ui->labTest->setText(tr("error: not the correct record class!!!"));
    }
...
void QFRDRDataEditor::rawDataChanged() {
	...
	ui->labTest->setText(tr("loaded record with files<br><i>%1</i><br><br><b>data: %2</b>").arg(m->getFiles().join("<br>")).arg(m->getData()));
}
