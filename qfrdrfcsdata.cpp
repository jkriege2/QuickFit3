...
void insertQFRDRFCSData(QFProject* project, ProgramOptions* options, QWidget* parent, QtLogFile* logWidget, QStatusBar* statusbar, QProgressBar* progressBar) {
    if (project) {
        QString alvf=parent->tr("ALV-5000 file (*.asc)");
        QString asciif=parent->tr("ASCII Data Files (*.txt *.dat *.csv)");
        QString currentFCSFileFormatFilter=options->getQSettings()->value("fcs/current_fcs_format_filter", alvf).toString();
        QStringList files = QFileDialog::getOpenFileNames(parent,
                              parent->tr("Select FCS Data File(s) to Import ..."),
                              options->getCurrentRawDataDir(),
                              alvf+";;"+asciif, &currentFCSFileFormatFilter);
        //std::cout<<"filter: "<<currentFCSFileFormatFilter.toStdString()<<std::endl;
        options->getQSettings()->setValue("fcs/current_fcs_format_filter", currentFCSFileFormatFilter);
        ...
        while(it != list.end()) {
            ...
            if (QFile::exists(*it)) {
                ...
                logWidget->log_text(parent->tr("loading [%2] '%1' ...\n").arg(*it).arg(currentFCSFileFormatFilter));
                if (currentFCSFileFormatFilter==alvf) {
                    unsigned int cc=1;
                    QString mode="";
                    ...
                    try {
                        ...
                        cc=0;
                        QMessageBox::critical(parent, parent->tr("QuickFit 3.0"), parent->tr("Error while importing ALV5000 file '%1':\n%2").arg(*it).arg(E.what()));
                        logWidget->log_error(parent->tr("Error while importing ALV5000 file '%1':\n    %2\n").arg(*it).arg(E.what()));
...
QList<QPair<QAction*, QFRawDataRecordFactory::QFRawDataRecordUserCreateFunc> > registerQFRDRFCSMenu(QWidget* parent, QMenu* menu) {
    ...
    QAction* actFCS=new QAction(QIcon(":/fcs_open.png"), parent->tr("Open FCS Data from File"), parent);
    actFCS->setStatusTip(parent->tr("Insert a new FCS data item from a specified file (CSV or ALV-5000 format)"));
    menu->addAction(actFCS);
    lst.append(qMakePair(actFCS, &insertQFRDRFCSData));
    return lst;
}
