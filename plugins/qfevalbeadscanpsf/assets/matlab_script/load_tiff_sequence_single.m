...
function [f, t]=load_tiff_sequence_single(filenam, valuesp)
...
if mode==0
    ...
    for k=1:N
        ...
        if (k==1) 
             ...
             f=single(zeros(inf.Width, inf.Height, N));
        end
        disp(['opening file ' [directory fn] ]);
        f(:,:,k)=single(imread([directory fn]));
    end
elseif mode==1
    ...
    if length(files)>1
        ...
        N=length(files);
        f=single(zeros(inf(1).Width, inf(1).Height, N));
        t=1:N;
        for k=1:N
            ...
            disp(['opening file ' [directory files(k).name] ', extracted t=' num2str(t(k)) ', width=' num2str(inf(1).Width) ', height=' num2str(inf(1).Height)]);
            f(:,:,k)=single(imread([directory files(k).name])');
        end
    elseif length(files)==1
        ...
        t=1:N;
        for k=1:N
            f(:,:,k)=single(imread([directory files(1).name], k)');
        end
    end
end
