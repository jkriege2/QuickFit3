...
#include <QDebug>
QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorACFR::QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorACFR() {
...
double QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorACFR::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=(  rho1  *QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor_2Dxy(a, 0, 0, D1, t, wxyR, wxyR, 0/*wzR*/, 0/*wzR*/)
                               + rho2  *QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor_2Dxy(a, 0, 0, D2, t, wxyR, wxyR, 0/*wzR*/, 0/*wzR*/));

    return offset+backfactor*(etar*etar*(cb+cab)+kappa*kappa*etag*etag*(ca+cab)+2.0*kappa*etag*etar*cab)*corrfunction/(Fr*Fr);

}

void QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorACFR::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorACFR::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorACFR::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorACFR::transformParametersForAdditionalPlot(int plot, double* params) const {
...
void QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorACFR::sortParameter(double *parameterValues, double *error, bool *fix) const {
