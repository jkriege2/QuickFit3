...
void QFFitFunctionsSPIMFCSDiffE2::calcParameter(double* data, double* error) const {
    ...
    // calculate Veff = pi^{3/2} * a^2 * sigmaz
    ...
    // calculate Aeff =  pi*a^2
    data[FCSSDiff_focus_area]=M_PI*a*a;
    if (error) error[FCSSDiff_focus_area]=sqrt(sqr(ea*2.0*M_PI*a));
...
unsigned int QFFitFunctionsSPIMFCSDiffE2::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCSDiffE2::transformParametersForAdditionalPlot(int plot, double* params) const {
