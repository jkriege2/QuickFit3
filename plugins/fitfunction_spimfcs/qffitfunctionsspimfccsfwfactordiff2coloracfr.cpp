...
#include <QDebug>
QFFitFunctionsSPIMFCCSFWFactorDiff2ColorACFR::QFFitFunctionsSPIMFCCSFWFactorDiff2ColorACFR() {
    ...
    #define FCCSDiff_diff_factorb 4
    ...
    #define FCCSDiff_diff_factorab 6
...
double QFFitFunctionsSPIMFCCSFWFactorDiff2ColorACFR::evaluate(double t, const double* data) const {
...
void QFFitFunctionsSPIMFCCSFWFactorDiff2ColorACFR::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsSPIMFCCSFWFactorDiff2ColorACFR::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsSPIMFCCSFWFactorDiff2ColorACFR::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCCSFWFactorDiff2ColorACFR::transformParametersForAdditionalPlot(int plot, double* params) const {
