...
QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorACFG::QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorACFG() {
...
double QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorACFG::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=(  rho1  *QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor_2Dxz(a, 0, 0, D1, t, wxyG, wxyG, wzG, wzG)
                               + rho2  *QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor_2Dxz(a, 0, 0, D2, t, wxyG, wxyG, wzG, wzG));

    return offset+backfactor*(etag*etag*ca+etag*etag*cab)*corrfunction/(Fg*Fg);

}

void QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorACFG::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
//        ecr2=error[FCCSDiff_count_rate2];
    ...
    data[FCSSDiff_focus_volume]=SPIMFCS_newAeff_xz(a, wxyG, wzG);
    if (error) error[FCSSDiff_focus_volume]=SPIMFCS_newAeffError_xz(a, ea, wxyG, ewxyG, wzG, ewzG);
...
bool QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorACFG::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorACFG::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorACFG::transformParametersForAdditionalPlot(int plot, double* params) const {
...
void QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorACFG::sortParameter(double *parameterValues, double *error, bool *fix) const {
