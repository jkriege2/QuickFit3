...
QFFitFunctionsSPIMFCCSFWSepDiff2ColorCCF::QFFitFunctionsSPIMFCCSFWSepDiff2ColorCCF() {
    ...
    #define FCCSDiff_count_rate1 19
    ...
    #define FCCSDiff_background1 20
    ...
    #define FCCSDiff_count_rate2 21
    ...
    #define FCCSDiff_background2 22
    ...
    #define FCCSDiff_brightness_a 23
    ...
    #define FCCSDiff_brightness_b 24

}


double QFFitFunctionsSPIMFCCSFWSepDiff2ColorCCF::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor(a, dx, dy, dz, D1, t, wxyG, wxyR, wzG, wzR);

    return offset+backfactor*(etag*etar*cab+kappa*etag*etar*ca+kappa*etag*etag*cab)*corrfunction/(Fg*Fr);
}

void QFFitFunctionsSPIMFCCSFWSepDiff2ColorCCF::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsSPIMFCCSFWSepDiff2ColorCCF::isParameterVisible(int parameter, const double* data) const {
    return true;
}

unsigned int QFFitFunctionsSPIMFCCSFWSepDiff2ColorCCF::getAdditionalPlotCount(const double* params) {
    return 0;
}

QString QFFitFunctionsSPIMFCCSFWSepDiff2ColorCCF::transformParametersForAdditionalPlot(int plot, double* params) {
    return "";
}
