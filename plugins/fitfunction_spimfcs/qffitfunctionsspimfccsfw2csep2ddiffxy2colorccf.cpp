...
QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorCCF::QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorCCF() {
    ...
    #define FCCSDiff_molbrightness_a 26
    ...
    #define FCCSDiff_molbrightness_b 27
}


double QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorCCF::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=(  rho1  *QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor_2Dxy(a, dx, dy, D1, t, wxyG, wxyR, 0/*wzG*/, 0/*wzR*/)
                               + rho2  *QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor_2Dxy(a, dx, dy, D2, t, wxyG, wxyR, 0/*wzG*/, 0/*wzR*/));

    return offset+backfactor*(etag*etar*cab+kappa*etag*etar*ca+kappa*etag*etag*cab)*corrfunction/(Fg*Fr);
}

void QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorCCF::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
//        eoffset=error[FCCSDiff_offset];
    ...
    data[FCSSDiff_focus_volume1]=SPIMFCS_newVeff(a, wxyG, 0/*wzG*/);
    if (error) error[FCSSDiff_focus_volume1]=SPIMFCS_newVeffError(a, ea, wxyG, ewxyG, 0/*wzG*/, 0/*ewzG*/);

    data[FCSSDiff_focus_volume2]=SPIMFCS_newVeff(a, wxyR, 0/*wzR*/);
    if (error) error[FCSSDiff_focus_volume2]=SPIMFCS_newVeffError(a, ea, wxyR, ewxyR, 0/*wzR*/, 0/*ewzR*/);
...
bool QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorCCF::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorCCF::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorCCF::transformParametersForAdditionalPlot(int plot, double* params) const {
...
void QFFitFunctionsSPIMFCCSFW2CSep2DDiffXY2ColorCCF::sortParameter(double *parameterValues, double *error, bool *fix) const {
