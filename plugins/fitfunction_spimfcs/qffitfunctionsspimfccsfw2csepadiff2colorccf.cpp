...
QFFitFunctionsSPIMFCCSFW2CSepADiff2ColorCCF::QFFitFunctionsSPIMFCCSFW2CSepADiff2ColorCCF() {
...
double QFFitFunctionsSPIMFCCSFW2CSepADiff2ColorCCF::evaluate(double t, const double* data) const {
    ...
    const double corrfunction= QFFitFunctionsSPIMFCCSFWADiff2ColorCCF_corrfactor(a, dx, dy, dz, GAMMA,alpha, t, wxyG, wxyR, wzG, wzR);

    return offset+backfactor*(etag*etar*cab+kappa*etag*etar*ca+kappa*etag*etag*cab)*corrfunction/(Fg*Fr);
}

void QFFitFunctionsSPIMFCCSFW2CSepADiff2ColorCCF::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsSPIMFCCSFW2CSepADiff2ColorCCF::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsSPIMFCCSFW2CSepADiff2ColorCCF::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCCSFW2CSepADiff2ColorCCF::transformParametersForAdditionalPlot(int plot, double* params) const {
