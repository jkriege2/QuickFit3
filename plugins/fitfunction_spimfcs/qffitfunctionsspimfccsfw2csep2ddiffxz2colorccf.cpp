...
QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorCCF::QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorCCF() {
    ...
    addParameter(FloatNumber,  "dissociation_constant",     "disscciation constant KD of reaction ab <-> a+b",      "K<sub>d</sub>",            "nmol/dm^2",           "nmol/dm<sup>2</sup>",    false,      false,          false,              QFFitFunction::DisplayError, false, 0.5,          0,        1e-50,     1     );
    ...
    #define FCSSDiff_focus_volume1 20
    ...
    #define FCSSDiff_focus_volume2 21
    ...
    #define FCCSDiff_brightness_a 26
    ...
    #define FCCSDiff_brightness_b 27
    ...
    #define FCCSDiff_molbrightness_a 28
    ...
    #define FCCSDiff_molbrightness_b 29
}


double QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorCCF::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=(  rho1  *QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor_2Dxz(a, dx, dz, D1, t, wxyG, wxyR, wzG, wzR)
                               + rho2  *QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor_2Dxz(a, dx, dz, D2, t, wxyG, wxyR, wzG, wzR));

    return offset+backfactor*(etag*etar*cab+kappa*etag*etar*ca+kappa*etag*etag*cab)*corrfunction/(Fg*Fr);
}

void QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorCCF::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
//        eoffset=error[FCCSDiff_offset];
    ...
    data[FCSSDiff_focus_volume1]=SPIMFCS_newAeff_xz(a, wxyG, wzG);
    if (error) error[FCSSDiff_focus_volume1]=SPIMFCS_newAeffError_xz(a, ea, wxyG, ewxyG, wzG, ewzG);

    data[FCSSDiff_focus_volume2]=SPIMFCS_newAeff_xz(a, wxyR, wzR);
    if (error) error[FCSSDiff_focus_volume2]=SPIMFCS_newAeffError_xz(a, ea, wxyR, ewxyR, wzR, ewzR);
...
bool QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorCCF::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorCCF::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorCCF::transformParametersForAdditionalPlot(int plot, double* params) const {
...
void QFFitFunctionsSPIMFCCSFW2CSep2DDiffXZ2ColorCCF::sortParameter(double *parameterValues, double *error, bool *fix) const {
