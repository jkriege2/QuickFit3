...
double QFFitFunctionsSPIMFCSADiff::evaluate(double t, const double* data) const {
    ...
    if (comp>0) {
        ...
        const double fac_z1=sqrt(1.0+GAMMA1*ta1/sqr(sigmaz));
        const double dt_sigma2=GAMMA1*ta1+sqr(wxy);
    ...
    if (comp>1) {
        ...
        const double fac_z2=sqrt(1.0+GAMMA2*ta2/sqr(sigmaz));
        const double dt_sigma22=GAMMA2*ta2+sqr(wxy);
    ...
    if (comp>2) {
        ...
        const double fac_z3=sqrt(1.0+GAMMA3*ta3/sqr(sigmaz));
        const double dt_sigma23=GAMMA3*ta3+sqr(wxy);
...
void QFFitFunctionsSPIMFCSADiff::calcParameter(double* data, double* error) const {
    ...
    data[SPIMFCSADIFF_focus_volume]=2.0*a*a*sigmaz;
    if (error) error[SPIMFCSADIFF_focus_volume]=sqrt(sqr(esigmaz*2.0*a*a)+sqr(ea*4.0*a*sigmaz));

    // calculate C = N / Veff
    if (data[SPIMFCSADIFF_focus_volume]!=0) data[SPIMFCSADIFF_concentration]=N/data[SPIMFCSADIFF_focus_volume]/(NAVOGADRO * 1.0e-24); else data[SPIMFCSADIFF_concentration]=0;
    if (error) {
        if ((sigmaz!=0)&&(a!=0)) error[SPIMFCSADIFF_concentration]=sqrt( sqr(eN/sqr(a)/2.0/sigmaz) + sqr(esigmaz*N/2.0/sqr(a)/sqr(sigmaz)) + sqr(ea*N/cube(a)/sigmaz) )/(NAVOGADRO * 1.0e-24);
        else error[SPIMFCSADIFF_concentration]=0;
    }
    ...
    double Aeff=data[SPIMFCSADIFF_focus_effA]=2.0*a*a;
    if (error) error[SPIMFCSADIFF_focus_effA]=0;
...
unsigned int QFFitFunctionsSPIMFCSADiff::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCSADiff::transformParametersForAdditionalPlot(int plot, double* params) const {
