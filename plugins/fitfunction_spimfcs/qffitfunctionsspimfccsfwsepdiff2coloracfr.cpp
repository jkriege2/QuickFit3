...
#include <QDebug>
QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFR::QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFR() {
    ...
    #define FCCSDiff_focus_height1 9
    ...
    #define FCCSDiff_focus_height2 11
    ...
    #define FCCSDiff_focus_width2 12
    ...
    #define FCSSDiff_focus_volume1 14
    ...
    #define FCSSDiff_focus_volume2 15
    ...
    #define FCCSDiff_count_rate2 18
    ...
    #define FCCSDiff_background2 19
    ...
    #define FCCSDiff_brightness_b 21
    ...
    #define FCCSDiff_molbrightness_a 22
    ...
    #define FCCSDiff_molbrightness_b 23

}


double QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFR::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor(a, 0, 0, 0, D1, t, wxyR, wxyR, wzR, wzR);

    return offset+backfactor*(etar*etar*(cb+cab)+kappa*kappa*etag*etag*(ca+cab)+2.0*kappa*etag*etar*cab)*corrfunction/(Fr*Fr);

}

void QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFR::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFR::isParameterVisible(int parameter, const double* data) const {
