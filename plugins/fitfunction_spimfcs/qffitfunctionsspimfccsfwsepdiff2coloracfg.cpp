...
QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFG::QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFG() {
    ...
    #define FCCSDiff_focus_height1 4
    ...
    #define FCCSDiff_brightness_a 10
    ...
    #define FCCSDiff_molbrightness_a 11

}


double QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFG::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor(a, 0, 0, 0, D1, t, wxyG, wxyG, wzG, wzG);

    return offset+backfactor*(etag*etag*ca+etag*etag*cab)*corrfunction/(Fg*Fg);

}

void QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFG::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFG::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFG::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsSPIMFCCSFWSepDiff2ColorACFG::transformParametersForAdditionalPlot(int plot, double* params) const {
