...
QFFCCSParameterInputTable::QFFCCSParameterInputTable(QFFCCSFitEvaluationItem *parent) :
...
Qt::ItemFlags QFFCCSParameterInputTable::flags(const QModelIndex &index) const
{
...
QVariant QFFCCSParameterInputTable::data(const QModelIndex &index, int role) const
{
    ...
    if (col==0) {
        if (row==0) {
            ...
            if (role==Qt::DisplayRole || role==Qt::EditRole) return fitparamids.value(row-2, QFFCCSParameterInputTable::FitParam()).label;
        }
...
QVariant QFFCCSParameterInputTable::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (role==Qt::DisplayRole) {
...
bool QFFCCSParameterInputTable::setData(const QModelIndex &index, const QVariant &value, int role)
{
    int col=index.column();
    int row=index.row();
...
void QFFCCSParameterInputTable::rebuildModel()
{
...
void QFFCCSParameterInputTable::setEditRanges(bool enabled)
{
    editRanges=enabled;
    rebuildModel();
}

bool QFFCCSParameterInputTable::checkRebuildModel(bool alwaysreset)
{
    QList<FitParam> fitparamids_new;
    bool ok=false;
    static int rebuildCount=0;
    QElapsedTimer t;
    //qDebug()<<"QFFCCSParameterInputTable::checkRebuildModel(): "<<rebuildCount+1;
    rebuildCount++;
    t.start();
    ...
    //qDebug()<<"QFFCCSParameterInputTable::checkRebuildModel() "<<rebuildCount<<": done "<<t.elapsed()<<"ms    =>"<<ok;
...
bool QFFCCSParameterInputTable::fitParamListContainsID(const QString &id, const QList<QFFCCSParameterInputTable::FitParam> &fitparamids, int* found)
{
...
QFFCCSParameterInputTable::FitParam::FitParam()
{
    id="";
    isFit=false;
    label="";
...
bool QFFCCSParameterInputTable::FitParam::isValid() const
{
    return !id.isEmpty();
}

bool QFFCCSParameterInputTable::FitParam::operator==(const QFFCCSParameterInputTable::FitParam &other) const
{
