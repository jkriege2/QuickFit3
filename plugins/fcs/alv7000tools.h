...
#ifndef ALV7000TOOLS_H
#define ALV7000TOOLS_H
...
/*
  Simple adaptation from alv5000tools.h
*/
/*!
    \defgroup quickfit2ALV ALV-7000 Data File (ACSII) Import
    \ingroup qf3rdrdp_fcs

    This group contains methods, structs and enums that are used to import ALV-7000 data files.

    \section Quickfit2_ALVFileFormat File Format of ALV-7000 data files (ASCII)
    ...
    The first line of the header identifies the file format and ALV software version (standard <tt>ALV-7000/E-WIN Data</tt>).
 ...
 *  \ingroup quickfit2ALV
 */
class ALV7_exception : public std::exception {
    private:
        std::string error;
    public:
        ALV7_exception(std::string error_desc) throw() { error=error_desc; std::cout<<error.c_str()<<std::endl; };
        virtual ~ALV7_exception() throw() {};
...
/** \brief read some parameters from the given ALV7000 file
 *  \ingroup quickfit2ALV
 *
 * \param filename filename of the ALV7000 file to be loaded
 * \param mode the mode string stored in the ALV7000 file
 ...
 */
void ALV7_analyze(QString filename, QString& mode, unsigned int& channelCount, unsigned int& runCount, bool& crossCorrelation, bool &autoCorrelation, int &inputchannels, int &firstchannel, int* corrColumns=NULL);
