...
QFFitFunctionFCCSNormalDiff2D2Focus::QFFitFunctionFCCSNormalDiff2D2Focus() {
    ...
    #define FCSDiff_focus_volume 12
    ...
    #define FCSDiff_cpm 20
}

QString QFFitFunctionFCCSNormalDiff2D2Focus::id() const {
    return QString("fccs_2f_diff2d");
}

QString QFFitFunctionFCCSNormalDiff2D2Focus::name() const {
    return  QString("2focus-FCS: 2D Normal Diffusion, D as parameter");
}
...
double QFFitFunctionFCCSNormalDiff2D2Focus::evaluate(double t, const double* data) const {
    ...
    double rho1=1.0;
    double d1=1.0/(1.0+relD1)*exp(-1.0*dxy2/(wxy2+4.0*D1*t));
    double d2=0;
    double d3=0;
    if (comp>1) {
        double relD2=0;
        if (D2!=0) relD2=4.0*D2*t/wxy2;
        d2=rho2/(1.0+relD2)*exp(-1.0*dxy2/(wxy2+4.0*D2*t));
        rho1=1.0-rho2;
    }
    if (comp>2) {
        double relD3=0;
        if (D3!=0) relD3=4.0*D3*t/wxy2;
        d3=rho3/(1.0+relD3)*exp(-1.0*dxy2/(wxy2+4.0*D3*t));
        rho1=1.0-rho2-rho3;
    }
...
void QFFitFunctionFCCSNormalDiff2D2Focus::calcParameter(double* data, double* error) const {
...
bool QFFitFunctionFCCSNormalDiff2D2Focus::isParameterVisible(int parameter, const double* data) const {
    int comp=data[FCSDiff_n_components];
    switch(parameter) {
