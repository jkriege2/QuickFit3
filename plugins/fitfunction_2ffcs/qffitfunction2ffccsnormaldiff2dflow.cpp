...
QString QFFitFunctionFCCSNormalDiff2DFlow2Focus::name() const {
    return  QString("2focus-FCCS: 2D Normal Diffusion + Flow, D,N,v as parameters");
}
double QFFitFunctionFCCSNormalDiff2DFlow2Focus::evaluate(double t, const double* data) const {
    ...
    const double D1=data[FCSDiff_diff_coeff1]; // in microm^2/s
...
unsigned int QFFitFunctionFCCSNormalDiff2DFlow2Focus::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionFCCSNormalDiff2DFlow2Focus::transformParametersForAdditionalPlot(int /*plot*/, double* /*params*/) const {
    return QString();
}
