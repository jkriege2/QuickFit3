...
QString QFFitFunctionFCCSNormalDiff3D2Focus::name() const {
    if (hasGamma) return  QString("2focus-FCCS: 3D Normal Diffusion, D, N, wxy and gamma as parameter");
    return QString("2focus-FCCS: 3D Normal Diffusion, D, N, wxy and wz as parameter");
}
...
unsigned int QFFitFunctionFCCSNormalDiff3D2Focus::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionFCCSNormalDiff3D2Focus::transformParametersForAdditionalPlot(int /*plot*/, double* /*params*/) const {
    return QString();
}
