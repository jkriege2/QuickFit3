...
QFFitFunctionFCCSAnomalousDiff3D2Focus::QFFitFunctionFCCSAnomalousDiff3D2Focus(bool hasGamma) {
    ...
    #define FCSDiff_diff_acoeff3 10
    ...
    #define FCSDiff_focus_distance 13
    ...
    if (hasGamma) {
        ...
        #define FCSDiff_focus_height 15
        ...
        #define FCSDiff_focus_struct_fac 16
    } else {
        addParameter(FloatNumber,  "focus_width",             "focus: lateral radius",                                 "w<sub>x,y</sub>",          "nm",         "nm",                     true,      true,         true,              QFFitFunction::EditError,    true, 250,          0,        1e4,      1    );
        #define FCSDiff_focus_width 14
...
QString QFFitFunctionFCCSAnomalousDiff3D2Focus::id() const {
    if (hasGamma) return QString("fccs_2f_adiff3d");
    return QString("fccs_2f_adiff3d_wz");
}

QString QFFitFunctionFCCSAnomalousDiff3D2Focus::name() const {
    if (hasGamma) return  QString("2focus-FCCS: 3D Anomalous Diffusion, Gamma, alpha, N, wxy and gamma as parameter");
    return QString("2focus-FCCS: 3D Anomalous Diffusion, Gamma, alpha, N, wxy and wz as parameter");
}

double QFFitFunctionFCCSAnomalousDiff3D2Focus::evaluate(double t, const double* data) const {
    const int comp=data[FCSDiff_n_components];
    const double N=data[FCSDiff_n_particle];
    const double G1=data[FCSDiff_diff_acoeff1]; // in µm^2/s
    const double a1=data[FCSDiff_diff_alpha1];
    const double rho2=data[FCSDiff_diff_rho2];
    const double G2=data[FCSDiff_diff_acoeff2];
    const double a2=data[FCSDiff_diff_alpha2];
    const double rho3=data[FCSDiff_diff_rho3];
    const double G3=data[FCSDiff_diff_acoeff3];
...
void QFFitFunctionFCCSAnomalousDiff3D2Focus::calcParameter(double* data, double* error) const {
...
bool QFFitFunctionFCCSAnomalousDiff3D2Focus::isParameterVisible(int parameter, const double* data) const {
    int comp=data[FCSDiff_n_components];
    switch(parameter) {
        case FCSDiff_diff_rho1:  return comp>1;
        case FCSDiff_diff_acoeff1: case FCSDiff_diff_alpha1: return comp>0;
        case FCSDiff_diff_rho2: case FCSDiff_diff_acoeff2: case FCSDiff_diff_alpha2: return comp>1;
        case FCSDiff_diff_rho3: case FCSDiff_diff_acoeff3: case FCSDiff_diff_alpha3: return comp>2;
    }
    return true;
}

unsigned int QFFitFunctionFCCSAnomalousDiff3D2Focus::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionFCCSAnomalousDiff3D2Focus::transformParametersForAdditionalPlot(int /*plot*/, double* /*params*/) const {
    return QString();
}
