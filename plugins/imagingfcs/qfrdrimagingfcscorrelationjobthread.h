...
/*! \brief this thread does all the correlation work
    ...
    The correlation process does this:
      - \b (1) open input file
      - \b (2) count the number of frames in the input file
      - \b (3) go through all frames and calculate the imager series statistics:
        - \b (3.1) one rame with the averaged grey value over eachpixel separately (average frame)
      ...
      .
      - \b (4) set background correction value (possibly as calculated in (3)
      - \b (5) calculate ACFs
      - \b (6) calculate the CCFs
      - \b (7) save results:
        - \b (7.1) average frame \c avgFrame as 16-bit uint TIFF  (\c basename.overview.tif ), with color scaling: smallest \c avgFrame value -> 0, largest \c avgFrame value -> 65535
        - \b (7.2) vidoe as multi-frame 16-bit uint TIFF (\c basename.video.tift ), with color scaling over ALL frames in video: smallest value -> 0, largest value -> 65535
        - \b (7.3) statistics as comma-separated file (\c basename.statistics.dat ) in the format \verbatim    time[seconds], average, stddev, min, max  \endverbatim
        - \b (7.4) ACF as comma-separated file (\c basename.autocorrelation.dat ) in the format \verbatim    lag time[seconds], acf, acf_error  \endverbatim The \c acf_error is optional.
        - \b (7.5) CCF as comma-separated file (\c basename.crosscorrelation.dat ) in the format \verbatim    lag time[seconds], ccf_left, ccf_right, ccf_top, ccf_bottom, ccf_left_error, ccf_right_error, ccf_top_error, ccf_bottom_error  \endverbatim The \c ccf_errors are optional.
        - \b (7.6) settings (\c basename.evalsettings.txt ) in the format \verbatim    settings_name   : value  \endverbatim
      .
    .
