...
#define LCD_BASEADRESS_LINE4     0x50
    ...
    //#define LCD_BASEADRESS_LINE1     0x00
    //#define LCD_BASEADRESS_LINE2     0x20
    //#define LCD_BASEADRESS_LINE3     0x40
    //#define LCD_BASEADRESS_LINE4     0x60
    #define LCD_BASEADRESS_LINE1     0x00
    #define LCD_BASEADRESS_LINE2     0x20
    #define LCD_BASEADRESS_LINE3     0x40
    #define LCD_BASEADRESS_LINE4     0x60
...
void lcd_init(char cursor_onoff, char cursor_blinking) {
#if (LCD_USED_CONTROLLER==LCD_CONTROLLER_HD44780) || (LCD_USED_CONTROLLER==LCD_CONTROLLER_KS0066)
    #warning "                 LCD library uses HD44780 or KS0066 controller"
    LCDPORTS_OUT();
    LCD_RS_DDR |= (1<<LCD_RS);
#if LCD_RW_CONNECTED!=0
    LCD_RW_DDR |= (1<<LCD_RW);
    ...
    if (LCD_DATASTART==0) {
        ...
        LCD_PORT &= 0x0F;
        LCD_PORT |= (0x03<<LCD_DATASTART);               // setzen
    }
    ...
    if (LCD_DATASTART==0) {
        ...
        LCD_PORT &= 0x0F;
        LCD_PORT |= (0x02<<LCD_DATASTART);               // setzen
    }
    lcd_enable();
    _delay_ms(5);
...
#elif (LCD_USED_CONTROLLER==LCD_CONTROLLER_KS0073)
    #warning "                 LCD library uses KS0073 controller"
    LCDPORTS_OUT();
    LCD_RS_DDR |= (1<<LCD_RS);
#if LCD_RW_CONNECTED!=0
    LCD_RW_DDR |= (1<<LCD_RW);
...
#if LCD_INTERFACE==LCD_INTERFACE_4BIT
    lcd_data_out4(0b00000010);
    _delay_ms(5);

    // 4Bit / 2 Zeilen / 5x7 / extended instruction set (RE=1)
    lcd_command(0b00100100);
    #if (LCD_LINECOUNT==4)&&(LCD_CHARACTERCOUNT>16)
    lcd_command(0b00001001);
    #endif
    // 4Bit / 2 Zeilen / 5x7 / normal instruction set (RE=0)
    ...
    _delay_ms(100);
        ...
        #warning "                 4 Bit Interface"
            ...
            lcd_data_out4(0x02); // function set 0b0010XXXX
            _delay_ms(10);
    ...
    uint8_t c=0x21;
        ...
        c=c + 0x08;
        #endif
        lcd_command(c);    // function set        0b0010<N><DH><IS2><IS1>
                           //                     0b0010 N  0   0    1    =0x21 & (N*0x08)
        lcd_command(0b00010101); // intern. osc freq.   0b0001<BS><F2><F1><F0>
                                 //                     0b0001 0   1   0   1    =0x15
        lcd_command(0b01010011); // power/ICON/contrast 0b0101<Ion><Bon><C5><C4>
                                 //                     0b0101 0    0    0   0    =0x50
        lcd_command(0b01101100); // follower control    0b0110<Fon><Rab2><Rab1><Rab0>
                                 //                     0b0110  1    1     0     0    =0x6C
        lcd_command(0b01111111); // contrast set        0b0111<C3><C2><C1><C0>
                                 //                     0b0111 1   1   0   0    =0x7C
    ...
    lcd_command(c & 0b11111100); // switch back to instruction table 0  0b0010<N>000  =0x38
...
void lcd_setdoubleheight(char doubleheight_line) {
    ...
    c=0x20;
            ...
            if (doubleheight_line==1) { // DH=1, N=1, UD=1
                lcd_command(c|0b00001110); // set DH and switch to table 10
                lcd_command(0x18); // set UD
                lcd_command(c|0b00001100); // set DH and switch to table 00
            } else if (doubleheight_line==2) { // DH=1, N=1, UD=0
                lcd_command(c|0b00001110); // set DH and switch to table 10
                lcd_command(0x10); // clear UD
                lcd_command(c|0b00001100); // set DH and switch to table 00
            } else { // DH=0, N=1, UD=0
                lcd_command(c|0b00001010); // clear DH and switch to table 10
                lcd_command(0x10); // clear UD
                lcd_command(c|0b00001000); // clear DH and switch to table 00
            }
        #endif
    #endif
#endif
...
void lcd_setcontrast(uint8_t contrast) {
    ...
    lcd_command(co|0x09);    // function set        0b0011<N><DH><IS2><IS1>
                          //                     0b0011 1  0   0    1    =0x39
    #if (LCD_VOLTAGE == LCD_VOLTAGE_5V)
    ...
    lcd_command(co|0x08); // switch back to instruction table 0  0b00111000  =0x38
#endif
}
...
void lcd_cursor(uint8_t x, uint8_t y) {
    ...
    default:
        tmp=LCD_INSTRUCTION_GOTO+LCD_BASEADRESS_LINE1+x;
        break;                    // für den Fall einer falschen Zeile
    }
    lcd_command(tmp);
    _delay_us(LCD_LONGDELAY);
}
...
void lcd_putc(char data) {
    if (data=='\f') {
        ...
        lcd_data(0x07);
    #if (LCD_USED_CONTROLLER != LCD_CONTROLLER_KS0066) && (LCD_USED_CONTROLLER != LCD_CONTROLLER_KS0073)
    } else if (data=='°') {
        lcd_data(0xDF);
    } else if (data=='|') {
        lcd_data(0x7C);
