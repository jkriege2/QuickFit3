...
ErrorModel::ErrorModel(SDFFLib::baseImporter* importer, QObject *parent)
...
void ErrorModel::loadErrors(SDFFLib::baseImporter* importer){
    events.clear();
    if (importer!=NULL) {
        if (importer->get_events_count()>0) for (unsigned int i=0; i<importer->get_events_count(); i++) {
            SDFFLib::bi_event e=importer->get_event(i);
            events.push_back(e);
        }
    }
    beginResetModel();
...
void ErrorModel::addEvent(SDFFLib::bi_event event) {
...
QVariant ErrorModel::data(const QModelIndex &index, int role) const{
    ...
    if (role == Qt::DisplayRole) {
        switch(index.column()) {
            case 0: {
                if (events[index.row()].type == SDFFLib::biInfo) return QString(tr("info"));
                if (events[index.row()].type == SDFFLib::biWarning) return QString(tr("warning"));
                if (events[index.row()].type == SDFFLib::biError) return QString(tr("error"));
                return QVariant();
            }
        ...
        switch(index.column()) {
            case 0: {
                if (events[index.row()].type == SDFFLib::biInfo) return QIcon(":/event_info.png");
                if (events[index.row()].type == SDFFLib::biWarning) return QIcon(":/event_warning.png");
                if (events[index.row()].type == SDFFLib::biError) return QIcon(":/event_error.png");
                return QVariant();
            }
            default: return QVariant();
        }
        ...
        switch(events[index.row()].type) {
            case SDFFLib::biInfo: return QColor("darkgreen");
            case SDFFLib::biError: return QColor("darkred");
            case SDFFLib::biWarning: return QColor("darkblue");
...
SDFFLib::bi_event ErrorModel::getEvent(unsigned int n){
    return events[n];
};
