 ...
 * .
 *
 * The SDFFLib::SDFFParser class produces (like all SDFFLib::baseImporter descendents) a list if SDFFLib::bi_event
 * objects that represent one error/message/warning event each. This class stores a list
 * of such SDFFLib::bi_event events in the private property list and allows for access to them via
...
public:
    ...
    ErrorModel(SDFFLib::baseImporter* importer = NULL, QObject *parent = 0);

    /** \brief load errors from SDFFLib::baseImporter class into this Qt model class */
    void loadErrors(SDFFLib::baseImporter* importer);

    /** \brief return the n-th event */
    SDFFLib::bi_event getEvent(unsigned int n);

    /** \brief add an addition event */
    void addEvent(SDFFLib::bi_event event);
...
private:
    ...
    std::vector<SDFFLib::bi_event> events;
};
