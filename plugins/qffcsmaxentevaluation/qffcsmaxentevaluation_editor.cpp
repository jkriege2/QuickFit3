...
#include "ui_qffcsmaxentevaluation_editor.h"
...
QFFCSMaxEntEvaluationEditor::QFFCSMaxEntEvaluationEditor(QFPluginServices* services, QWidget* parent):
    ...
    ui(new Ui::QFFCSMaxEntEvaluationEditor)
{
    updatingData=true;
...
void QFFCSMaxEntEvaluationEditor::readSettings() {
    ...
    if (!settings) return;
    currentSaveDirectory=settings->getQSettings()->value(QString("fcs_maxent/editor/lastSaveDirectory"), currentSaveDirectory).toString();
};

void QFFCSMaxEntEvaluationEditor::writeSettings() {
    // write widget settings
    if (!settings) return;
    settings->getQSettings()->setValue(QString("fcs_maxent/editor/lastSaveDirectory"), currentSaveDirectory);
}
...
void QFFCSMaxEntEvaluationEditor::displayEvaluation() {
    if (!current) return;
...
void QFFCSMaxEntEvaluationEditor::doEvaluation(QFRawDataRecord* record) {
...
void QFFCSMaxEntEvaluationEditor::evaluateCurrent() {
...
void QFFCSMaxEntEvaluationEditor::evaluateAll() {
