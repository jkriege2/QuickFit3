...
#define REG_UCSRA UCSR1A
#define REG_UCSRB UCSR1B
#define REG_UCSRC UCSR1C
#define REG_UDR UDR1
#define REG_UBRRL UBRR1L
#define REG_UBRRH UBRR1H
#define REG_UBRR UBRR1

#define BIT_UDRE UDRE1
#define BIT_UDRIE UDRIE1
#define BIT_RXEN RXEN1
#define BIT_TXEN TXEN1
#define BIT_RXCIE RXCIE1
#define BIT_RUPM1 RUPM11
#define BIT_UPM0 UPM10
#define BIT_UPM1 UPM11
#define BIT_URSEL URSEL1
#define BIT_UCSZ0 UCSZ10
#define BIT_UCSZ1 UCSZ11
#define BIT_UCSZ2 UCSZ12
...
#define BUFSIZE_OUT 32
uint8_t outbuf1[BUFSIZE_OUT];
fifo_t outfifo1;

int uart1_putc (const uint8_t c) {
...
void uart1_flush(void) {
    ...
    fifo_init (&infifo1,   inbuf1, BUFSIZE_IN);
}


void uart1_init(uint32_t baudrate, uint8_t databits, uint8_t stopbits, uint8_t parity) {
    // set baud rate
    uint8_t sreg = SREG;
    ...
    fifo_init (&infifo1,   inbuf1, BUFSIZE_IN);
    fifo_init (&outfifo1, outbuf1, BUFSIZE_OUT);
...
SIGNAL (SIG_UART_RECV) {
    _inline_fifo_put (&infifo1, REG_UDR);
}
...
SIGNAL (SIG_UART_DATA) {
    if (outfifo1.count > 0)
        REG_UDR = _inline_fifo_get (&outfifo1);
    else
...
int uart1_getc_nowait (void) {
    return fifo_get_nowait (&infifo1);
}

uint8_t uart1_getc_wait (void) {
    return fifo_get_wait (&infifo1);
}

void uart1_puts_P (PGM_P s) {
    while (1) {
