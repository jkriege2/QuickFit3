...
#  define BAUDRATE_LASER 19200
#  define LASER_DELAY_MS 30


uint8_t laser_recv(char* answer, int answermax, char* answer_start) {
    if (!laser_connected()) {
        ledpow_off();
        ledlaser_off();
        ledlock_off();
    ...
    uint32_t t0=time_counter+3;
	//do {
        int i=0;
        char c=-1;
        for (i=0; i<answermax; i++) answer[i]=0;
        i=0;
        while (time_counter<t0 && ((i<=0)||(c!=10)) && i<answermax-1) {
            c=uart_getc_nowait();
            if (c>0) {
                answer[i]=c;
                i++;
	...
	//} while ((str_startswith(answer, answer_start)==0) && (time_counter<t0));
#ifdef ENABLEDEBUGMODE
	if (DEBUG_MODE!=0) {
		fprintf_P(UART, PSTR("D> Laser answers (%d chars): %s\n"), strlen(answer), answer);
...
void laser_ini(uint8_t* ERROR, uint16_t* MAX_POWER, double* WAVELENGTH) {
        ...
        uart_flush();
        uart_puts("?MAXLP\r\n");
        _delay_ms(2*LASER_DELAY_MS);
    #ifdef ENABLEDEBUGMODE
        if (DEBUG_MODE!=0) {
            uart1_puts_P(PSTR("D> sent ?MAXLP\n"));
        }
    #endif
        if (laser_recv(answer, 30, "MAXLP=")!=0) {
            *MAX_POWER=(uint16_t)(atof(&(answer[6])));
    #ifdef ENABLEDEBUGMODE
            if (DEBUG_MODE!=0) {
                uart1_puts_P(PSTR("D> cut answer: "));
                uart1_puts(&(answer[6]));
                uart1_puts("\n");
            }
    #endif
    ...
    #ifdef ENABLEDEBUGMODE
        if (DEBUG_MODE!=0) {
            uart1_puts_P(PSTR("D> sent ?WAVE\n"));
        }
    #endif
        if (laser_recv(answer, 30, "WAVE=")!=0) {
            *WAVELENGTH=atoi(&(answer[5]));
    #ifdef ENABLEDEBUGMODE
            if (DEBUG_MODE!=0) {
                uart1_puts_P(PSTR("D> cut answer: "));
                uart1_puts(&(answer[5]));
...
#ifdef ENABLEDEBUGMODE
	if (DEBUG_MODE!=0) {
		uart1_puts_P(PSTR("D> sent ?P\n"));
	}
#endif
    if (laser_recv(answer, 30, "P=")!=0) {
        if (answer[0]=='P' && answer[1]=='=' && strlen(answer)>2) *POW_MEAS=atof(&(answer[2]));
#ifdef ENABLEDEBUGMODE
        if (DEBUG_MODE!=0) {
            uart1_puts_P(PSTR("D> cut answer: "));
            uart1_puts(&(answer[2]));
...
void laser_send_pow(uint8_t* ERROR, uint8_t LASER_ON, uint16_t POWER, uint8_t set_leds, double* POW_MEAS) {
    ...
    if (LASER_ON!=0) {
		...
		_delay_ms(LASER_DELAY_MS);
        fprintf(UART_LASER, "L=1\r\n");
#ifdef ENABLEDEBUGMODE
		if (DEBUG_MODE!=0) {
			uart1_puts_P(PSTR("D> sent L=1\n"));
		}
#endif
		_delay_ms(LASER_DELAY_MS);
        laser_recv(answer, 30, NULL);

		_delay_ms(LASER_DELAY_MS*2);


        fprintf(UART_LASER, "P=%f\r\n", (float)POWER/10.0);
...
#endif
        laser_recv(answer, 30, "P=");
        if (answer[0]=='P' && answer[1]=='=' && strlen(answer)>2) *POW_MEAS=atof(&(answer[2]));

    } else {
#ifdef ENABLEDEBUGMODE
		if (DEBUG_MODE!=0) {
			uart1_puts_P(PSTR("D> sent L=0\n"));
		}
#endif
        fprintf(UART_LASER, "L=0\r\n");
		if (set_leds) ledlaser_off();
		_delay_ms(LASER_DELAY_MS);
        laser_recv(answer, 30, "L=");
		_delay_ms(LASER_DELAY_MS);
...
char* laser_getadditionalinfo(void) {
    ...
    sprintf_P(las_addinfo, PSTR("w=%4.1fnm  "), WAVELENGTH);
    return las_addinfo;
}
