...
QFFitFunctionsTIRFFCCSFW2CSep2DDiffXY2ColorACFG::QFFitFunctionsTIRFFCCSFW2CSep2DDiffXY2ColorACFG() {
...
double QFFitFunctionsTIRFFCCSFW2CSep2DDiffXY2ColorACFG::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=(  rho1  *QFFitFunctionsTIRFCCSFWDiff2ColorCCF_corrfactor_2Dxy(a, 0, 0, D1, t, wxyG, wxyG)
                               + rho2  *QFFitFunctionsTIRFCCSFWDiff2ColorCCF_corrfactor_2Dxy(a, 0, 0, D2, t, wxyG, wxyG));

    return offset+backfactor*(etag*etag*ca+etag*etag*cab)*corrfunction/(Fg*Fg);

}

void QFFitFunctionsTIRFFCCSFW2CSep2DDiffXY2ColorACFG::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsTIRFFCCSFW2CSep2DDiffXY2ColorACFG::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsTIRFFCCSFW2CSep2DDiffXY2ColorACFG::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsTIRFFCCSFW2CSep2DDiffXY2ColorACFG::transformParametersForAdditionalPlot(int plot, double* params) const {
...
void QFFitFunctionsTIRFFCCSFW2CSep2DDiffXY2ColorACFG::sortParameter(double *parameterValues, double *error, bool *fix) const {
