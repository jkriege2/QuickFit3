...
QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorCCF::QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorCCF() {
    ...
    #define FCCSDiff_diff_acoeffa 8
    ...
    #define FCCSDiff_diff_acoeffb 10
    ...
    #define FCCSDiff_diff_alphab 11
    ...
    #define FCCSDiff_diff_acoeffab 12
    ...
    #define FCCSDiff_diff_alphaab 13
    ...
    #define FCCSDiff_focus_distancex 16
    ...
    #define FCCSDiff_focus_distancey 17
...
double QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorCCF::evaluate(double t, const double* data) const {
    ...
    // double Grr_b=etaR*etaR*cb*QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor(a, 0, 0, 0, Db, t, wxyR, wxyR);
    // double Grr_ab=etaR*etaR*cab*QFFitFunctionsSPIMFCCSFWDiff2ColorCCF_corrfactor(a, 0, 0, 0, Dab, t, wxyR, wxyR);
     ...
     double Ggr_ab=etaG*etaR*cab*QFFitFunctionsTIRFCCSFWADiff2ColorCCF_corrfactor_2Dxy(a, dx, dy, Dab, alphaab, t, wxyG, wxyR);
...
void QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorCCF::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorCCF::isParameterVisible(int parameter, const double* data) const {
    return true;
}

unsigned int QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorCCF::getAdditionalPlotCount(const double* params) {
    return 0;
}

QString QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorCCF::transformParametersForAdditionalPlot(int plot, double* params) {
    return "";
}

void QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorCCF::sortParameter(double *parameterValues, double *error, bool *fix) const {
