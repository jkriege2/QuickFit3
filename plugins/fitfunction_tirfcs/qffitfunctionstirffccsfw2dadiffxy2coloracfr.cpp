...
#include <QDebug>
QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorACFR::QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorACFR() {
...
double QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorACFR::evaluate(double t, const double* data) const {
    ...
    const double Fr=etaR*(cb+cab)+kappa*Fg;
     ...
     double Grr_b=etaR*etaR*cb*QFFitFunctionsTIRFCCSFWADiff2ColorCCF_corrfactor_2Dxy(a, 0, 0, Db, alphab, t, wxyR, wxyR);
     double Grr_ab=etaR*etaR*cab*QFFitFunctionsTIRFCCSFWADiff2ColorCCF_corrfactor_2Dxy(a, 0, 0, Dab, alphaab, t, wxyR, wxyR);
...
void QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorACFR::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorACFR::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorACFR::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsTIRFFCCSFW2DADiffXY2ColorACFR::transformParametersForAdditionalPlot(int plot, double* params) const {
