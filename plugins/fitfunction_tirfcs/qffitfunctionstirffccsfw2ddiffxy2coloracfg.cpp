...
QFFitFunctionsTIRFFCCSFW2DDiffXY2ColorACFG::QFFitFunctionsTIRFFCCSFW2DDiffXY2ColorACFG() {
...
double QFFitFunctionsTIRFFCCSFW2DDiffXY2ColorACFG::evaluate(double t, const double* data) const {
...
void QFFitFunctionsTIRFFCCSFW2DDiffXY2ColorACFG::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsTIRFFCCSFW2DDiffXY2ColorACFG::isParameterVisible(int parameter, const double* data) const {
...
unsigned int QFFitFunctionsTIRFFCCSFW2DDiffXY2ColorACFG::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsTIRFFCCSFW2DDiffXY2ColorACFG::transformParametersForAdditionalPlot(int plot, double* params) const {
...
void QFFitFunctionsTIRFFCCSFW2DDiffXY2ColorACFG::sortParameter(double */*parameterValues*/, double */*error*/, bool */*fix*/) const {
