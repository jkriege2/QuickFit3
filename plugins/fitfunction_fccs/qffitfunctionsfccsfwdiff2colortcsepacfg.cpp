...
#include <cmath>
#include "fcstools.h"
QFFitFunctionsFCCSFWDiff2ColorTCSepACFG::QFFitFunctionsFCCSFWDiff2ColorTCSepACFG() {
    ...
    #define FCCSDiff_focus_structfac1 12
...
double QFFitFunctionsFCCSFWDiff2ColorTCSepACFG::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=QFFitFunctionFCCSFWTriplet(nNonFl, t, thetaT, tauT)
                              *(  rho1  *QFFitFunctionsFCCSFWDiff2ColorCCF_corrfactor(0, 0, 0, D1, t, wxyG, wxyG, wzG, wzG)
                                + rho2  *QFFitFunctionsFCCSFWDiff2ColorCCF_corrfactor(0, 0, 0, D2, t, wxyG, wxyG, wzG, wzG));

    return offset+backfactor*(etag*etag*ca+etag*etag*cab)*corrfunction/(Fg*Fg);
}

void QFFitFunctionsFCCSFWDiff2ColorTCSepACFG::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsFCCSFWDiff2ColorTCSepACFG::isParameterVisible(int parameter, const double* data) const {
    if (data) {
        switch(parameter) {
...
unsigned int QFFitFunctionsFCCSFWDiff2ColorTCSepACFG::getAdditionalPlotCount(const double* /*params*/) const {
    return 1;
}

QString QFFitFunctionsFCCSFWDiff2ColorTCSepACFG::transformParametersForAdditionalPlot(int /*plot*/, double* params) const {
    params[FCCSDiff_n_nonfluorescent]=0;
    return QObject::tr("only diffusion");
}


void QFFitFunctionsFCCSFWDiff2ColorTCSepACFG::sortParameter(double *parameterValues, double *error, bool *fix) const {
