...
#include "fcstools.h"
#include <QDebug>
QFFitFunctionsFCCSFWDiff2ColorSepACFR::QFFitFunctionsFCCSFWDiff2ColorSepACFR() {
    ...
    #define FCCSDiff_focus_structfac2 14
    ...
    #define FCCSDiff_focus_width2 15
...
double QFFitFunctionsFCCSFWDiff2ColorSepACFR::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=QFFitFunctionFCCSFWTriplet(nNonFl, t, thetaT, tauT)
                                *QFFitFunctionsFCCSFWDiff2ColorCCF_corrfactor(0, 0, 0, D1, t, wxyR, wxyR, wzR, wzR);

    return offset+backfactor*(etar*etar*(cb+cab)+kappa*kappa*etag*etag*(ca+cab)+2.0*kappa*etag*etar*cab)*corrfunction/(Fr*Fr);
}

void QFFitFunctionsFCCSFWDiff2ColorSepACFR::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsFCCSFWDiff2ColorSepACFR::isParameterVisible(int parameter, const double* data) const {
    if (data) {
        switch(parameter) {
...
unsigned int QFFitFunctionsFCCSFWDiff2ColorSepACFR::getAdditionalPlotCount(const double* params) {
    return 1;
 }

QString QFFitFunctionsFCCSFWDiff2ColorSepACFR::transformParametersForAdditionalPlot(int plot, double* params) {
    params[FCCSDiff_n_nonfluorescent]=0;
    return QObject::tr("only diffusion");
}


void QFFitFunctionsFCCSFWDiff2ColorSepACFR::sortParameter(double *parameterValues, double *error, bool *fix) const {
