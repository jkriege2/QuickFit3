...
#include <cmath>
#include "fcstools.h"
QFFitFunctionsFCCSFWDiff2ColorTCSepCCF::QFFitFunctionsFCCSFWDiff2ColorTCSepCCF() {
    ...
    #define FCCSDiff_diff_coeff2 13
    ...
    #define FCCSDiff_focus_distancez 18
    ...
    #define FCCSDiff_focus_structfac1 19
    ...
    #define FCCSDiff_focus_width1 20
    ...
    #define FCCSDiff_focus_structfac2 21
    ...
    #define FCCSDiff_focus_width2 22
...
double QFFitFunctionsFCCSFWDiff2ColorTCSepCCF::evaluate(double t, const double* data) const {
    ...
    const double corrfunction=QFFitFunctionFCCSFWTriplet(nNonFl, t, thetaT, tauT)
                              *(  rho1  *QFFitFunctionsFCCSFWDiff2ColorCCF_corrfactor(dx, dy, dz, D1, t, wxyG, wxyR, wzG, wzR)
                                + rho2  *QFFitFunctionsFCCSFWDiff2ColorCCF_corrfactor(dx, dy, dz, D2, t, wxyG, wxyR, wzG, wzR));

    return offset+backfactor*(etag*etar*cab+kappa*etag*etar*ca+kappa*etag*etag*cab)*corrfunction/(Fg*Fr);

}

void QFFitFunctionsFCCSFWDiff2ColorTCSepCCF::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsFCCSFWDiff2ColorTCSepCCF::isParameterVisible(int parameter, const double* data) const {
    if (data) {
        switch(parameter) {
...
unsigned int QFFitFunctionsFCCSFWDiff2ColorTCSepCCF::getAdditionalPlotCount(const double* /*params*/) const {
    return 1;
}

QString QFFitFunctionsFCCSFWDiff2ColorTCSepCCF::transformParametersForAdditionalPlot(int /*plot*/, double* params) const {
    params[FCCSDiff_n_nonfluorescent]=0;
    return QObject::tr("only diffusion");
}

void QFFitFunctionsFCCSFWDiff2ColorTCSepCCF::sortParameter(double *parameterValues, double *error, bool *fix) const {
