...
#include <cmath>
#include "fcstools.h"
QFFitFunctionsFCCSFWDiff2ColorCCF::QFFitFunctionsFCCSFWDiff2ColorCCF() {
    ...
    #define FCCSDiff_diff_coeffb 14
    ...
    #define FCCSDiff_diff_coeffab 15
...
double QFFitFunctionsFCCSFWDiff2ColorCCF::evaluate(double t, const double* data) const {
...
void QFFitFunctionsFCCSFWDiff2ColorCCF::calcParameter(double* data, double* error) const {
    const double cab=data[FCCSDiff_concentrationab]*6.022e-1; // FCCSDiff_concentrationab given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double ecab=0;
    const double ca=data[FCCSDiff_concentrationa]*6.022e-1; // FCCSDiff_concentrationa given in 1e-9*6.022e23 particles/litre but c should be in particles per µm³= particles/10^{-15}litres=1e15 particles/litre
    double eca=0;
...
bool QFFitFunctionsFCCSFWDiff2ColorCCF::isParameterVisible(int parameter, const double* data) const {
    if (data) {
        switch(parameter) {
...
unsigned int QFFitFunctionsFCCSFWDiff2ColorCCF::getAdditionalPlotCount(const double* params) const {

    Q_UNUSED(params);
    return 0;
}

QString QFFitFunctionsFCCSFWDiff2ColorCCF::transformParametersForAdditionalPlot(int plot, double* params) const {
