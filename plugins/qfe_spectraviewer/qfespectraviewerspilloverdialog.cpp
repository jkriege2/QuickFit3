...
void QFESpectraViewerSpilloverDialog::init(const QList<QFESpectraViewerPlotItem> &plotItems, SpectrumManager *manager)
{
    ...
    for (int i=0; i<plotItems.size(); i++) {
        ...
        } else if (plotItems[i].type==qfesFilterBandpass) {
    ...
    for (int i=0; i<fluorophores.size(); i++) {
        ...
        for (int j=0; j<filters.size(); j++) {
            ...
            if (manager->spectrumExists(fluorophores[i].spectrum_fl) && manager->spectrumExists(filters[i].spectrum)) {
                model.setCellCreate(i, c, QString::number(manager->getSpectrum(fluorophores[i].spectrum_fl)->getMulSpectrumIntegral(manager->getSpectrum(filters[i].spectrum))/manager->getSpectrum(fluorophores[i].spectrum_fl)->getSpectrumFullIntegral()*100.0)+QString(" %"));
            }
            c++;
        }
