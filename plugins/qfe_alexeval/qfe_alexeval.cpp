...
QFEAlexEval::QFEAlexEval(QObject* parent):
    QObject(parent)
{
	logService=NULL;
}

QFEAlexEval::~QFEAlexEval() {
...
void QFEAlexEval::initExtension() {
    ...
    actStartPlugin=new QFActionWithNoMenuRole(QIcon(getIconFilename()), tr("Start ALEX Evaluation (for spFRET)"), this);
...
void QFEAlexEval::loadSettings(ProgramOptions* settingspo) {
...
void QFEAlexEval::storeSettings(ProgramOptions* settingspo) {
...
void QFEAlexEval::log_text(QString message) {
	if (logService) logService->log_text(LOG_PREFIX+message);
	else if (services) services->log_text(LOG_PREFIX+message);
}

void QFEAlexEval::log_warning(QString message) {
	if (logService) logService->log_warning(LOG_PREFIX+message);
	else if (services) services->log_warning(LOG_PREFIX+message);
}

void QFEAlexEval::log_error(QString message) {
