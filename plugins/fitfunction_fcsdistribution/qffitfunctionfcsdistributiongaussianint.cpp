...
QFFitFunctionFCSDistributionIntGaussian::QFFitFunctionFCSDistributionIntGaussian() {
    ...
    #define FCSDLG_concentration 13
...
QVector<QPair<double, double> >  QFFitFunctionFCSDistributionIntGaussian::fillTauVal(double tau_min, double tau_max, uint32_t tau_values, double tauC, double tauSigma) const {
...
double QFFitFunctionFCSDistributionIntGaussian_f(double x, void * params) {
    ...
    return exp(-0.5*qfSqr((x-p->tauC)/p->tauSigma))/(1.0+p->tau/x)/sqrt(1.0+p->tau/x/p->gamma/p->gamma)/sqrt(2.0*M_PI*qfSqr(p->tauSigma));
}
...
double QFFitFunctionFCSDistributionIntGaussian::evaluate(double t, const double* data) const {
    ...
    if (N>0) {
        ...
        QFFitFunctionFCSDistributionIntGaussian_intparam p = {t, tauD1, tauD1_sigma, gamma};
        p.gamma=gamma;
        p.tau=t*1e6;
        p.tauC=tauD1*1e6;
        p.tauSigma=tauD1_sigma*1e6;
