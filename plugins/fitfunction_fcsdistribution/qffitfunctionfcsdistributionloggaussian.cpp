...
QFFitFunctionFCSDistributionLogGaussian::QFFitFunctionFCSDistributionLogGaussian() {
    ...
    addParameter(FloatNumber,  "diff_tau1",               "center diffusion time of distribution",                 "&tau;<sub>D,c</sub>",      "usec",        "&mu;s",                 true,      true,         true,              QFFitFunction::DisplayError, 30,           1,        1e10,     1,   0        );
    #define FCSDLG_diff_tau1 7
    addParameter(FloatNumber,  "b",                       "width parameter distribution",                          "b",                        "ln(sec)",    "ln(sec)",                true,      true,         true,              QFFitFunction::DisplayError, 1,            1e-10,    1e10,     1,   0);
    #define FCSDLG_b 8
    ...
    addParameter(FloatNumber,  "tau_range_min",           "smallest evaluated diffusion time",                     "&tau;<sub>min</sub>",      "usec",       "&mu;s",                  false,    false,        false,              QFFitFunction::NoError,      1,            0,        1e50,     10,  0    );
    #define FCSDLG_tau_range_min 13
    addParameter(FloatNumber,  "tau_range_max",           "largest evaluated diffusion time",                      "&tau;<sub>max</sub>",      "usec",       "&mu;s",                  false,    false,        false,              QFFitFunction::NoError,      1e7,          0,        1e50,     10,  0    );
    ...
    addParameter(FloatNumber,  "diff_coeff1",             "center diffusion coefficient of distriubution",         "D<sub>c</sub>",            "micron^2/s", "&mu;m<sup>2</sup>/s",    false,    false,        false,              QFFitFunction::DisplayError, 500,          0,        1e50,     1    );
...
QVector<QPair<double, double> >  QFFitFunctionFCSDistributionLogGaussian::fillTauVal(double tau_min, double tau_max, double logc, double b) const {
    ...
    while (tau<=tau_max) {
        ...
        double val=exp(-0.5*sqr((log(tau)-logc)/b));
        sum=sum+val;
