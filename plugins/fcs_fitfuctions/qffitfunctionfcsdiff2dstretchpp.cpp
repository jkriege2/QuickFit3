...
QFFitFunctionFCSDiff2DStretchPP::QFFitFunctionFCSDiff2DStretchPP(bool isTIRF) {
    this->isTIRF=isTIRF;
    ...
    addParameter(FloatNumber,  "nonfl_beta1",             "triplet stretch parameter",                             "&beta;<sub>trip</sub>",    "",            "",                      true,      true,         true,              QFFitFunction::DisplayError, false, 1.0,          1e-10,    1000,     0.1  );
    #define FCSDiff_nonfl_beta1 3
    ...
    #define FCSDiff_nonfl_tau2 5
    addParameter(FloatNumber,  "nonfl_beta2",             "dark state stretch parameter",                          "&beta;<sub>dark,2</sub>",  "",            "",                      true,      true,         true,              QFFitFunction::DisplayError, false, 1.0,          1e-10,    1000,     0.1  );
    #define FCSDiff_nonfl_beta2 6
...
double QFFitFunctionFCSDiff2DStretchPP::evaluate(double t, const double* data) const {
    ...
    double pre=1.0;
    if (nonfl_comp==1) {
        pre=(1.0-nf_theta1+nf_theta1*exp(-pow(t/nf_tau1, nf_beta1)))/(1.0-nf_theta1);
    } else if (nonfl_comp==2) {
        pre=(1.0-nf_theta1+nf_theta1*exp(-pow(t/nf_tau1, nf_beta1))-nf_theta2+nf_theta2*exp(-pow(t/nf_tau2, nf_beta2)))/(1.0-nf_theta1-nf_theta2);
    }
...
void QFFitFunctionFCSDiff2DStretchPP::calcParameter(double* data, double* error) const {
...
bool QFFitFunctionFCSDiff2DStretchPP::isParameterVisible(int parameter, const double* data) const {
    int comp=data[FCSDiff_n_components];
    int nonfl_comp=data[FCSDiff_n_nonfluorescent];
    switch(parameter) {
...
unsigned int QFFitFunctionFCSDiff2DStretchPP::getAdditionalPlotCount(const double* /*params*/) const {
    return 1;
}

QString QFFitFunctionFCSDiff2DStretchPP::transformParametersForAdditionalPlot(int plot, double* params) const {
    if (plot==0) {
