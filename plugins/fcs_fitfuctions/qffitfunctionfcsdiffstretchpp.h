...
#ifndef QFFitFunctionFCSDiffStretchPP_H
#define QFFitFunctionFCSDiffStretchPP_H
...
/*! \brief plugin class for FCS Normal Diffusion Fit Functions with stretched photophysics
    ...
    This class implements a 3D diffusion model for the fitting routines in QuickFit2. The model may be denoted as:
      \f[ G(\tau)=G_\infty+\frac{1}{N}\cdot\frac{1}{(1+b/\mbox{cps})^2}\cdot\left(\frac{1-\Theta_{non}+\Theta_{non}\mathrm{e}^{-(\tau/\tau_{non})^{\beta_{non}}}-\Theta_{trip}+\Theta_{trip}\mathrm{e}^{-(\tau/\tau_{trip})^{\beta_{trip}}}}{1-\Theta_{non}-\Theta_{trip}}\right)\cdot\left[(1-\rho_2-\rho_3)\cdot g_1(\tau)+\rho_2\cdot g_2(\tau)+\rho_3\cdot g_3(\tau)\right] \f]
    with
    ...
    The parameters are:
      ...
      - \f$ \beta_{trip}, \beta_{non} \f$: stretching parameters for blinking
...
*/
class QFFitFunctionFCSDiffStretchPP: public QFFCSFitFunctionBase {
    public:
...
#endif // QFFitFunctionFCSDiffStretchPP_H
