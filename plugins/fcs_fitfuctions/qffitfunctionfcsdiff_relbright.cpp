...
void QFFitFunctionFCSDiffDifferentBrightness::calcParameter(double* data, double* error) const {
...
bool QFFitFunctionFCSDiffDifferentBrightness::isParameterVisible(int parameter, const double* data) const {
    int comp=data[FCSDiff_n_components];
    int nonfl_comp=data[FCSDiff_n_nonfluorescent];
    switch(parameter) {
...
unsigned int QFFitFunctionFCSDiffDifferentBrightness::getAdditionalPlotCount(const double* /*params*/) const {
    return 1;
}

QString QFFitFunctionFCSDiffDifferentBrightness::transformParametersForAdditionalPlot(int plot, double* params) const {
