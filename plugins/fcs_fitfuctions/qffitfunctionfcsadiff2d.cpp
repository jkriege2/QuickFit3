...
QFFitFunctionFCSADiff2D::QFFitFunctionFCSADiff2D(bool isTIRF) {
    ...
    #define FCSADiff_focus_width 18
    ...
    #define FCSADiff_cpm 22
}

double QFFitFunctionFCSADiff2D::evaluate(double t, const double* data) const {
...
void QFFitFunctionFCSADiff2D::calcParameter(double* data, double* error) const {
    ...
    data[FCSADiff_focus_volume]=M_PI*sqr(wxy);
    if (error) error[FCSADiff_focus_volume]=sqrt(sqr(ewxy*2.0*M_PI*wxy));
...
bool QFFitFunctionFCSADiff2D::isParameterVisible(int parameter, const double* data) const {
    int comp=data[FCSADiff_n_components];
    int nonfl_comp=data[FCSADiff_n_nonfluorescent];
    switch(parameter) {
...
unsigned int QFFitFunctionFCSADiff2D::getAdditionalPlotCount(const double* /*params*/) const {
    return 1;
}

QString QFFitFunctionFCSADiff2D::transformParametersForAdditionalPlot(int /*plot*/,  double* params) const {
