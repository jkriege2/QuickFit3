...
bool QFRawDataRecordFactory::registerPlugin(const QString& filename_in, QObject *plugin, QFPluginHelpData &helpdata)
{
    ...
    if (iRecord) {
        ...
        addRDRPlugins(QFileInfo(filename_in).absoluteFilePath(), iRecord->getAdditionalRDRPlugins());
        return true;
    }
    return false;
}
...
void QFRawDataRecordFactory::finalizePluginSearch()
{
