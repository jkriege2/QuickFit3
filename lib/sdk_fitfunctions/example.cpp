...
class LIB_EXPORT ExampleFitFunction: public QFCFitFunction {
    public:
        ...
        bool isParameterVisible(int parameter, const double* data) const {
            return true;
            // all parameters are visible at all times
        }

        unsigned int getAdditionalPlotCount(const double* params) {
            return 1;
            // we have one additional plot
        }

        std::string transformParametersForAdditionalPlot(int plot, double* params) {
            params[PARAM1]=1;
            return std::string("p1=1");
