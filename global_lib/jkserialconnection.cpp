...
std::string JKSCdatabits2string(JKSCdatabits databits) {
    return inttostr(databits);
}

JKSCdatabits string2JKSCdatabits(std::string databits) {
    std::string d=strstrip(databits);
    if (d=="5") return JKSC5databits;
    if (d=="6") return JKSC6databits;
    if (d=="7") return JKSC7databits;
    if (d=="8") return JKSC8databits;
#if defined(__WINDOWS__) || defined(CS9)
    if (d=="9") return JKSC9databits;
#endif
    return JKSC8databits;
}

std::string JKSChandshaking2string(JKSChandshaking d) {
    if (d==JKSCnoHandshaking) return "no_handshaking";
    if (d==JKSCxonXoffHandshaking) return "xonxoff_handshaking";
    if (d==JKSChardwareHandshakingDTRRTS) return "hardware_handshaking";
    return "";
}

JKSChandshaking string2JKSChandshaking(std::string dd) {
    std::string d=tolower(strstrip(dd));
    if (d=="no_handshaking" || d=="none") return JKSCnoHandshaking;
    if (d=="xonxoff_handshaking" || d=="xonxoff") return JKSCxonXoffHandshaking;
    if (d=="hardware_handshaking" || d=="hardware") return JKSChardwareHandshakingDTRRTS;
    return JKSCnoHandshaking;
}

std::string JKSCparity2string(JKSCparity d) {
    if (d==JKSCevenParity) return "even_parity";
    if (d==JKSCoddParity) return "odd_parity";
    if (d==JKSCnoParity) return "no_parity";
    return "";
}

JKSCparity string2JKSCparity(std::string dd) {
    std::string d=tolower(strstrip(dd));
    if (d=="no_parity" || d=="none") return JKSCnoParity;
    if (d=="odd_parity" || d=="odd") return JKSCoddParity;
    if (d=="even_parity" || d=="even") return JKSCevenParity;
    return JKSCnoParity;
}

std::string JKSCstopbits2string(JKSCstopbits d) {
    if (d==JKSConeStopbit) return "one_stopbit";
    if (d==JKSCone5Stopbits) return "one5_stopbits";
    if (d==JKSCtwoStopbits) return "two_stopbits";
    return "";
}

JKSCstopbits string2JKSCstopbits(std::string dd) {
    std::string d=tolower(strstrip(dd));
    if (d=="one_stopbit" || d=="1" || d=="one") return JKSConeStopbit;
    if (d=="one5_stopbit" || d=="1.5" || d=="one5") return JKSCone5Stopbits;
    if (d=="two_stopbit" || d=="2" || d=="two") return JKSCtwoStopbits;
    return JKSConeStopbit;
}


#ifdef __WINDOWS__
void JKSerialConnection::win32_error(std::string where) {
...
JKSerialConnection::JKSerialConnection(std::string port, unsigned int baudrate, JKSCdatabits databits, JKSCparity parity, JKSCstopbits stopbits, JKSChandshaking handshaking) {
    logToFile=false;
    log=NULL;
...
JKSerialConnection::~JKSerialConnection() {
    close();
    if (logToFile && (log==NULL)) log=fopen("rs232.log", "w");
    if (logToFile) fclose(log);
}

bool JKSerialConnection::open(std::string port, unsigned int baudrate, JKSCdatabits databits, JKSCparity parity, JKSCstopbits stopbits, JKSChandshaking handshaking) {
    this->port=port;
    this->baudrate=baudrate;
    this->databits=databits;
    this->parity=parity;
...
bool JKSerialConnection::reopen() {
...
bool JKSerialConnection::open() {

    //std::cout<<"opening port ...\n";
    std::string name="JKSerialConnection::open(port='"+port+"', baud_rate="+inttostr(baudrate)+", databits="+inttostr(databits)+" parity="+JKSCparity2string(parity)+", stopbits="+JKSCstopbits2string(stopbits)+" handshaking="+JKSChandshaking2string(handshaking)+")";
...
bool JKSerialConnection::write(std::string data) {
    if (logToFile && (log==NULL)) log=fopen("rs232.log", "w");
    if (logToFile) {
...
bool JKSerialConnection::read(uint8_t *data, size_t num_bytes) {
...
std::string JKSerialConnection::read(size_t num_bytes, bool* ok) {
    std::string name="JKSerialConnection::read(num_bytes='"+inttostr(num_bytes)+"')";
#ifdef __WINDOWS__
    std::string res="";
    unsigned long dwNumberOfBytesRecvd = 0;
...
bool JKSerialConnection::read(char* ch) {
...
bool JKSerialConnection::read_nowait(char *ch) {
    std::string name=std::string("JKSerialConnection::read(char* ch)");
    bool ok=true;
#ifdef __WINDOWS__
    char buf[3]={0,0,0};
...
std::string JKSerialConnection::readUntil(std::string end_string, bool* ok) {
...
int JKSerialConnection::readUntil(char *result, int result_maxsize, const char *end_string, int end_string_size, bool *ok)
{
...
std::string JKSerialConnection::readUntil(char end_char, bool* ok) {
...
bool JKSerialConnection::clearBuffer() {
...
std::vector<std::string> JKSerialConnection::listPorts() {
...
void JKSerialConnection::resetError()
{
    errorOccured=false;
    lastError="";
}
