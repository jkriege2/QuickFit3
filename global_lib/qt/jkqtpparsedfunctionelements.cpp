...
double JKQTPxParsedFunctionLineGraphFunction(double x, void* data) {
    ...
    if (d && d->parser && d->node) {
        try {
            ...
            jkMathParser::jkmpResult r=d->node->evaluate();
    ...
    JKQTPxFunctionLineGraph(parent)
{
    ...
    efdata.parser=new jkMathParser();
    efdata.node=NULL;
    efdata.varcount=0;
    errorFunction="";
...
void JKQTPxParsedFunctionLineGraph::createPlotData(bool /*collectParams*/)
{
    ...
    for (int i=0; i<fdata.varcount; i++) {
        fdata.parser->deleteVariable(std::string("p")+inttostr(i+1));
    }
    fdata.varcount=0;
    try {
        if (parent && parameterColumn>=0) {
            ...
            for (int i=imin; i<imax; i++) {
                ...
                fdata.parser->addVariableDouble(std::string("p")+inttostr(fdata.varcount+1), xv);
                fdata.varcount=fdata.varcount+1;

            }
        }
        for (int i=0; i<parameters.size(); i++) {
            fdata.parser->addVariableDouble(std::string("p")+inttostr(fdata.varcount+1), parameters[i]);
            fdata.varcount=fdata.varcount+1;
        }
    ...
    for (int i=0; i<efdata.varcount; i++) {
        efdata.parser->deleteVariable(std::string("p")+inttostr(i+1));
    }
    efdata.varcount=0;
    try {
        if (parent && errorParameterColumn>=0) {
            ...
            for (int i=imin; i<imax; i++) {
                ...
                efdata.parser->addVariableDouble(std::string("p")+inttostr(efdata.varcount+1), xv);
                efdata.varcount=efdata.varcount+1;

            }
        }
        for (int i=0; i<errorParameters.size(); i++) {
            efdata.parser->addVariableDouble(std::string("p")+inttostr(efdata.varcount+1), errorParameters[i]);
            efdata.varcount=efdata.varcount+1;
        }
