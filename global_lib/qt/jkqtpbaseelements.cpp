...
QString JKQTPcoordinateAxis::floattolabel(double data) {
    ...
    switch(labelType) {
        ...
        case JKQTPCALTexponent: {
                return QString(floattolatexstr(data, past_comma, remove_trail0, 1e-300, pow(10, -past_comma), pow(10, past_comma+1)).c_str());
            }; break;
        case JKQTPCALTexponentCharacter: {
                return QString(floattounitstr(data, past_comma, remove_trail0).c_str());
            }; break;
        case JKQTPCALTdate: {
                QDateTime dt;
...
QString JKQTPcoordinateAxis::floattolabel(double data, int past_comma) {
    ...
    if (labelType==JKQTPCALTdefault) {
        ...
        return res;
    } else if (labelType==JKQTPCALTexponent) return QString(floattolatexstr(data, past_comma, remove_trail0, 1e-300, pow(10, -past_comma), pow(10, past_comma+1)).c_str());
    else if (labelType==JKQTPCALTexponentCharacter) return QString(floattounitstr(data, past_comma, remove_trail0).c_str());
    return "";
}
