...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaat(QString("JKQtBasePlotter[%1]::calcPlotScaling()").arg(objectName()));
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot("JKQtBasePlotter::plotSystemGrid");
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot("JKQtBasePlotter::plotSystemXAxis");
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot("JKQtBasePlotter::plotSystemYAxis");
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot("JKQtBasePlotter::plotKey");
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot("JKQtBasePlotter::plotOverlays");
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot("JKQtBasePlotter::paintPlot");
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot("JKQtBasePlotter::gridPaint");
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot(QString("JKQtBasePlotter::draw(rect, %1)").arg(drawOverlays));
#endif
    bool oldEmitPlotSignals=emitPlotSignals;
    emitPlotSignals=false;
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot(QString("JKQtBasePlotter::draw(pos, %1)").arg(drawOverlays));
#endif
    bool oldEmitPlotSignals=emitPlotSignals;
    emitPlotSignals=false;
#ifdef JKQTBP_DEBUGTIMING
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot(QString("JKQtBasePlotter::drawNonGrid(rect, %1)").arg(drawOverlays));
#endif
    bool oldEmitPlotSignals=emitPlotSignals;
    emitPlotSignals=false;
#ifdef JKQTBP_DEBUGTIMING
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot(QString("JKQtBasePlotter::drawNonGrid(pos, %1)").arg(drawOverlays));
#endif
    bool oldEmitPlotSignals=emitPlotSignals;
    emitPlotSignals=false;
#ifdef JKQTBP_DEBUGTIMING
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot(QString("JKQtBasePlotter::drawNonGridOverlays(point)"));
#endif
    QRect rect=QRect(pos, QSize(widgetWidth/paintMagnification, widgetHeight/paintMagnification));
    painter.save();
    painter.translate(rect.topLeft());
...
void JKQtBasePlotter::saveData(QString filename, QString format) {
    ...
    if (fmt.isEmpty()) {
        std::string e=tolower(extract_file_ext(fn.toStdString()));
        if (e=="csv" || e=="dat") {
            fmt="csv";
        } else if (e=="txt") {
            fmt="tab";
...
void JKQtBasePlotter::saveImage(QString filename, bool displayPreview) {
    ...
    if (!fn.isEmpty()) {
        ...
        if (!isWithSpecialDeviceAdapter) {
            for (int i=0; i<jkqtpPaintDeviceAdapters.size(); i++) {
                if (jkqtpPaintDeviceAdapters[i]->getFileExtension().contains(e.c_str())) {
                    adapterID=i;
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot(QString("JKQtBasePlotter::plotGraphs"));
#endif
...
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot(QString("JKQtBasePlotter::plotKeyContents(%1, %2, %3, %4)").arg(x).arg(y).arg(width).arg(height));
#endif
    painter.save();
...
void JKQtBasePlotter::getKeyExtent(JKQTPEnhancedPainter& painter, double* width, double* height, double* text_width, double* text_height, int* columns_count, int *lines_count) {
#ifdef JKQTBP_AUTOTIMER
    JKAutoOutputTimer jkaaot("JKQtBasePlotter::getKeyExtent");
#endif
    QFont f=painter.font();
    f.setFamily(keyFont);
    f.setPointSizeF(keyFontSize*fontSizeMultiplier);
    ...
    if (keyLayout==JKQTPkeyLayoutOneColumn) {
        ...
        columns=mmin(columns, keyHeight);
        int lines=(int)ceil((double)keyHeight/(double)columns);
        lines=mmin(lines, keyHeight);
