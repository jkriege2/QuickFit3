...
void ALV_LoadFile(QString filename, quickfitDataStore* datastore) {
    ...
    while (token.type!=ALV_EOF && readingHeader) {
        /*switch (token.type) {
            case ALV_EOF: std::cout<<"EOF     "; break;
            case ALV_LINEBREAK: std::cout<<"LINEBR. "; break;
            case ALV_COLON: std::cout<<"COLON   "; break;
            case ALV_NAME: std::cout<<"NAME    "; break;
            case ALV_QUOTED: std::cout<<"QUOTED  "; break;
            case ALV_VALUE: std::cout<<"VALUE   "; break;
        }
        std::cout<<"'"<<token.value.toStdString()<<"'   "<<token.doubleValue<<std::endl;*/
        if (findIdentifier) {
            ...
            if (token.type==ALV_NAME) { // here we parse "<name> : <value>"
                ...
                if (item.name.compare("Date",  Qt::CaseInsensitive)==0) {
                    ...
                    /*std::cout<<"channel found "<<item1->channel.toStdString()<<std::endl;
                    if (item1->isCrossCorrelation) std::cout<<"cross"<<std::endl; else std::cout<<"auto"<<std::endl;
                    if (item1->isDualChannel) std::cout<<"dual"<<std::endl; else std::cout<<"single"<<std::endl;*/
